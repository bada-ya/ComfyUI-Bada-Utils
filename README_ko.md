...
└── 9. 🕵️ 바다 미싱 노드 탐정 & 깃허브 해결사 (MissingNodeDetective)
```
...
### 9. 🕵️ 바다 미싱 노드 탐정 & 깃허브 해결사 (`MissingNodeDetective`)
- **무슨 기능인가요?**: 
  - 타인의 워크플로우를 열었을 때 설치되지 않아 빨간색 'X'로 깨진 **미싱 노드(Missing Node)**를 실시간 자동 감지하여 노드 우측 하단에 고가시성 네온 탐정 뱃지(`🕵️ [바다] 깃허브/노드 찾기`)를 도킹합니다.
  - 클릭 한 번으로 ComfyUI 매니저의 40,000+ 노드 맵 및 최신 Nodes 2.0 DB를 초고속 6단계로 분석하여 **정확한 깃허브 원작자 저장소와 커스텀 노드 패키지를 1초 만에 찾아 원클릭으로 직접 설치**합니다.
  - `Label (rgthree)` 노드 등 투명 알파(`#fff0`)가 순정 ComfyUI의 불투명도 알고리즘에 의해 눈부신 새하얀 직사각형으로 왜곡되는 코어 버그를 100% 자동 치료하고, 원작자의 거대한 폰트와 스타일을 그대로 살려주는 우아한 렌더러를 제공합니다.
- **이럴 때 쓰면 편합니다**: 
  - Civitai나 웹에서 다운받은 워크플로우를 열었는데 처음 보는 미싱 노드가 빨갛게 떠서 어떤 커스텀 노드를 설치해야 할지 막막할 때
  - 매니저 검색이나 깃허브 검색을 일일이 돌아다니지 않고 캔버스에서 클릭 한 번으로 즉시 노드를 설치하고 싶을 때
  - 다운받은 워크플로우 속 커다란 텍스트 라벨 노드가 새하얗게 변색되어 캔버스를 가릴 때

<details>
<summary><b>📖 자세한 설명 및 설정 방법 (클릭하여 펼치기) ▼</b></summary>

#### 🌟 핵심 세부 기능
1. **🏷️ 미싱 노드 도킹 네온 뱃지 (`🕵️ [바다] 깃허브/노드 찾기`)**:
   * 워크플로우에 설치되지 않은 미싱 노드가 존재하면, 노드 우측 하단에 시선 집중 네온 뱃지가 자동으로 정밀 도킹됩니다.
   * 클릭 시 전용 분석 창이 즉각 팝업되며, 캔버스 우클릭 메뉴에서도 `[바다 탐정] 미싱 노드 분석 & 깃허브 찾기`를 즉시 호출할 수 있습니다.
2. **🧠 매니저 4만 노드 + Nodes 2.0 패키지 초고속 6-Tier 매칭 엔진**:
   * `Tier 1 (정확한 클래스명 일치)` ➔ `Tier 2 (네임스페이스 분해 매칭)` ➔ `Tier 3 (핵심 키워드 토큰 분할)` ➔ `Tier 4 (패키지 타이틀 일치)` ➔ `Tier 5 (매니저 nodename_pattern 정규식)` ➔ `Tier 6 (GitHub Stars 인기도 가중치 순위)` 순으로 0.001초 만에 최적의 저장소를 판별합니다.
   * 백엔드가 준비되지 않은 상태에서도 브라우저 프론트엔드가 ComfyUI Manager 공식 엔드포인트(`/v2/customnode/getmappings`)를 직접 조회하는 **이중 안전망(Dual-Layer Fallback)**을 갖추고 있습니다.
3. **⚡ 원클릭 직접 설치 & 매니저 자동 연동**:
   * **원클릭 설치**: 백엔드 REST API를 통해 `git clone` 및 `pip install -r requirements.txt`를 원클릭으로 백그라운드 자동 실행.
   * **ComfyUI 매니저 연동**: `[ 🧩 ComfyUI 매니저에서 보기 ]` 클릭 시 신형/구형 매니저 창을 자동으로 열어주고 해당 노드 검색어를 검색창에 자동 입력 및 복사해 줍니다.
4. **🔍 미등록 노드 검색 UX (Google & GitHub 코드 검색)**:
   * 매니저에 아직 등록되지 않은 최신/희귀 노드의 경우, 클래스명 기준 1순위 추천 **Google 검색**과 **GitHub 정확 일치 코드 검색** 바로가기를 제공하여 1초 만에 원작자 깃허브로 이동할 수 있습니다.
5. **🎨 `Label (rgthree)` 투명도 버그 완치 & 우아한 폰트 렌더러**:
   * `rgthree`의 투명 텍스트(`#fff0`)가 순정 ComfyUI의 색상 보정기에 의해 `0.95` 불투명 흰색으로 왜곡되던 코어 버그를 프로토타입 후킹으로 완벽 차단합니다.
   * 원작자가 설정한 폰트 크기(129px/37px 등), 컬러, 여백, 정렬을 캔버스 위에서 우아하게 그대로 렌더링하면서, 우측 하단 네온 뱃지로 원클릭 설치를 지원합니다.
6. **🚀 제로 오버헤드 최적화 (0% CPU)**:
   * 미싱 노드가 없는 일반 작업 환경에서는 마우스 이동 시 연산을 0회로 즉각 탈출(`Early Return`)하며, `requestAnimationFrame` 스로틀링을 적용하여 캔버스 성능에 미치는 영향을 0으로 유지합니다.
...
| **10. 미싱 노드 탐정** | **🕵️ 바다 미싱 노드 탐정 & 깃허브 해결사** | 캔버스 미싱 노드 네온 탐정 뱃지 표시 및 깃허브 원클릭 분석/설치 기능 활성화 여부를 설정합니다. |
