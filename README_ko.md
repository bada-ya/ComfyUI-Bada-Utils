...
├── 3. 🩺 노드 스마트 케어 (모델 자동 장착 & 미싱 노드 복구) (Node Smart Care)
...
└── 8. 🛡️ PrimeVue 툴팁 버그 자동 치료기 (Tooltip Healer)
```
...
### 3. 🩺 노드 스마트 케어 (`Node Smart Care`)
- **무슨 기능인가요?**: 
  - 외부 워크플로우를 열었을 때 발생하는 **미설치 커스텀 노드(빨간 X)**와 **누락된 모델/LoRA(빨간 테두리)** 문제를 단 하나의 깔끔한 우클릭 메뉴(`🩺 [바다] 노드 스마트 케어`)로 완벽히 통합 해결합니다.
  - 사용자가 클릭한 위치와 노드의 상태를 스스로 분석하여 **4가지 상황별 맞춤 라우팅**을 스마트하게 제공합니다:
    1. **빈 캔버스 우클릭**: 워크플로우 전체를 스캔하여 3개 탭(`📦 모델 & LoRA 자동 장착`, `🧩 미설치 미싱 노드 케어`, `🌐 전체 보기`)으로 구성된 종합 진단 모달을 띄웁니다.
    2. **미싱 노드(빨간 X) 우클릭**: 4만여 개 매니저 맵을 6단계로 초고속 분석하여 깃허브 저장소를 찾고, 원클릭 매니저 설치 카드를 단독 제공합니다.
    3. **정상 설치된 모델 노드 우클릭**: 내 PC 모델 폴더 트리 탐색기와 유사도(%) 기반 매칭을 띄워 1초 만에 모델을 자동 장착하고 빨간 에러 테두리를 즉시 제거합니다.
    4. **모델 슬롯을 가진 미싱 노드 우클릭**: 노드가 설치되지 않으면 모델을 끼울 수 없으므로, 커스텀 노드 설치가 최우선(Priority 1)으로 자동 실행됩니다.
  - 동일 패키지에 속한 여러 미싱 노드는 하나만 설치해도 나머지 노드 카드가 **`[✅ 이미 설치한 노드입니다 (동시 해결)]`**로 실시간 자동 연동됩니다.
  - `Label (rgthree)` 노드의 투명 알파(`#fff0`)가 순정 ComfyUI에 의해 새하얀 직사각형으로 왜곡되는 버그를 치료하며, 캔버스에 거추장스러운 뱃지 없이 100% 깨끗한 화면을 유지합니다.
- **이럴 때 쓰면 편합니다**: 
  - Civitai나 웹에서 다운로드한 워크플로우를 열었는데 처음 보는 미싱 노드가 빨갛게 깨지고 모델도 누락되었을 때
  - 여러 메뉴를 헤매지 않고 우클릭 한 번으로 커스텀 노드 설치와 모델 장착을 한자리에서 끝내고 싶을 때

<details>
<summary><b>📖 자세한 설명 및 설정 방법 (클릭하여 펼치기) ▼</b></summary>

#### 🎯 4가지 상황별 지능형 우클릭 라우팅 (`🩺 [바다] 노드 스마트 케어`)

| 구분 | 우클릭 대상 | 실행 동작 및 인터페이스 |
| :--- | :--- | :--- |
| **Case 1: 워크플로우 전체 진단** | 빈 캔버스 우클릭 | 3단 탭 워크플로우 종합 진단 모달 (`📦 모델 자동 장착`, `🧩 미싱 노드`, `🌐 전체 보기`) |
| **Case 2: 특정 미싱 노드 집중** | 빨간 'X' 미싱 노드 우클릭 | 해당 미싱 노드 1개만 단독 분석하여 깃허브 저장소 발굴 및 원클릭 매니저 설치 카드 노출 |
| **Case 3: 특정 모델 노드 집중** | 정상 설치된 모델/LoRA 노드 우클릭 | 해당 노드의 로컬 폴더 트리 탐색기 및 유사도(%) 자동 장착 인터페이스 단독 노출 |
| **Case 4: 모델 슬롯 보유 미싱 노드** | 미싱 노드 중 모델 입력이 있는 노드 우클릭 | **Priority 1 (우선순위 1)**: 노드 복구가 선행되어야 하므로 미싱 노드 해결사가 자동 활성화 |

#### 🌟 핵심 세부 기능
1. **🧩 3단 탭 워크플로우 닥터 (최상단 스크롤 완벽 보장)**:
   * `[📦 모델 & LoRA 자동 장착]`, `[🧩 미설치 미싱 노드 케어]`, `[🌐 전체 보기]` 탭을 원클릭으로 쾌적하게 전환.
   * `scrollTop = 0` 보장 기술로 탭 전환 시나 모달 생성 시 하단으로 튕기는 현상을 원천 방지.
2. **🧠 4만 매니저 노드 + Nodes 2.0 초고속 6-Tier 매칭 엔진**:
   * `Tier 1 (정확한 클래스명)` ➔ `Tier 2 (네임스페이스 분해)` ➔ `Tier 3 (키워드 토큰)` ➔ `Tier 4 (패키지명)` ➔ `Tier 5 (정규식 패턴)` ➔ `Tier 6 (깃허브 스타 가중치)` 순으로 0.001초 만에 최적의 저장소를 발굴.
   * 백엔드 일시 중단 시에도 브라우저에서 매니저 `/v2/customnode/getmappings`를 직접 질의하는 이중 안전망(Dual-Layer Fallback) 탑재.
3. **⚡ 원클릭 백그라운드 설치 & 동일 패키지 실시간 동시 해결 (`Already Installed`)**:
   * **원클릭 직접 설치**: 백엔드 REST API를 통해 `git clone` 및 `pip install`을 백그라운드로 자동 실행.
   * **실시간 동시 동기화**: 한 패키지에 포함된 여러 노드 중 1개만 설치해도, 동일 저장소의 다른 모든 카드 버튼이 실시간으로 `[✅ 이미 설치한 노드입니다 (동시 해결)]`로 일괄 잠금 전환되어 중복 설치 원천 차단.
4. **🌲 윈도우 탐색기형 폴더 트리 & 퍼지 유사도(%) 추천**:
   * 모델 디렉토리 계층 구조를 시원하게 탐색하고, 정밀도(`fp8`, `bf16`, `fp16`)와 버전(`v1`, `v2`, `turbo`)을 자동 정규화하여 내 PC 최적의 모델을 유사도 순으로 추천.
   * `Power Lora Loader (rgthree)`, `DaSiWa`, `Deno`, `Comfyroll`, `Efficiency Nodes` 등 서드파티 멀티 LoRA 노드 완벽 지원.
5. **🎨 `Label (rgthree)` 투명도 버그 완치 & 우아한 폰트 렌더러**:
   * `rgthree` 투명 텍스트(`#fff0`)가 순정 ComfyUI에 의해 새하얗게 변색되던 코어 버그를 프로토타입 후킹으로 완벽 방어.
   * 원작자의 거대 폰트 크기(129px, 37px 등), 컬러, 정렬을 캔버스 위에 우아하게 보존.
6. **🚀 캔버스 뱃지 제로 & 무부하 최적화 (0% Idle CPU)**:
   * 캔버스 위에 거추장스러운 뱃지를 일절 부착하지 않고 깨끗한 순정 화면을 유지하며, 우클릭 시에만 가동되는 100% 이벤트 드리븐 구조로 CPU 낭비 제로 달성.
...
### 8. 🛡️ PrimeVue 툴팁 버그 자동 치료기 (`Tooltip Healer`)
- **무슨 기능인가요?**: ComfyUI 환경에서 발생하는 화면 좌측 상단 `(0, 0)` 위치의 유령 풍선말 버그를 실시간으로 자동 감지하여 완벽하게 제거 및 위치를 교정합니다.
- **이럴 때 쓰면 편합니다**: 서드파티 스크립트의 전역 이벤트 캡처로 인해 빈 툴팁 팝업이 화면 좌상단 `(0, 0)`에 영구 고정되어 캔버스 작업 화면을 가릴 때 사용합니다.

<details>
<summary><b>📖 자세한 설명 및 설정 방법 (클릭하여 펼치기) ▼</b></summary>

#### 🌟 핵심 세부 기능
1. **🛡️ 화면 좌측 상단 (0, 0) 유령 풍선말 버그 100% 자동 치료 (`Tooltip Fixer`)**:
   * ComfyUI에서 특정 확장 노드가 로드될 경우, `common.js`의 전역 이벤트 간섭으로 인해 PrimeVue 툴팁(`.p-tooltip`)이 화면 좌측 상단 `(0, 0)` 좌표에 고정된 채 사라지지 않는 렌더링 버그가 발생합니다.
...
| **4. 캔버스 편의성** | **🖱️ 마우스 휠 줌 & 중간 버튼(휠) 패닝 보정기** | 텍스트 입력창이나 서드파티 노드 위에서도 마우스 휠 줌 및 중간 버튼 패닝이 멈추지 않고 매끄럽게 동작하도록 보정합니다. |
| **5. 시작 환경** | **🧼 시작 시 클린 빈 캔버스로 열기** | ComfyUI 최초 실행 및 새 탭 오픈 시 기본 모델 누락 오류("2 errors found")를 방지하고 깨끗한 빈 캔버스로 기동하도록 설정합니다. |
| **6. 프리셋 관리** | **🗃️ 글로벌 프리셋 & 인라인 요약 패널** | 노드 상단 지붕에 바로가기 뱃지 표시 및 워크플로우 간 전역 공유되는 노드별 프리셋을 한눈에 파악하고 백업/복원할 수 있는 통합 패널을 제공합니다. |
| **7. 이미지 편의성** | **📋 클립보드 & LoadImage 자동 에러 해결사** | 클립보드 이미지(`Ctrl+V`)를 붙여넣거나 하위 경로 로드 시 발생하는 빨간 테두리 에러를 자동으로 치료합니다. |
| **8. 노드 스마트 케어** | **🩺 노드 스마트 케어 (미싱 노드 복구 & 모델 자동 장착)** | 캔버스 빈 공간 또는 노드 우클릭 시 설치되지 않은 미싱 노드(빨간 X)와 누락된 모델/LoRA를 3단 탭 및 4가지 상황별 스마트 라우팅으로 진단하여 원클릭 설치 및 자동 장착합니다. |
