...
class BadaRegionalPrompt:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "ui_language": (["English", "한국어"], {"default": "English"}),
            },
            "optional": {
    ...
    def generate(self, prompt_text, format, aspect_ratio, grid_cols, grid_rows, ui_language="English", grid_data="{}", prefix_prompt="", suffix_prompt=""):
        ...
        total_cols = int(grid_cols)
        total_rows = int(grid_rows)
        final_prompt = ""
        ...
        if grid_data and grid_data.strip() and grid_data.strip() != "{}":
            try:
                ...
                raw_char_profile = data.get("char_profile", "").strip()
                char_profile = translate_prompt_to_english(raw_char_profile) if raw_char_profile else ""
                grid_borders = bool(data.get("grid_borders", True))

                if "areas" in data and isinstance(data["areas"], list) and len(data["areas"]) > 0:
                    has_parsed_areas = True
                    areas = data["areas"]
                    # Sort areas top-to-bottom, left-to-right
                    sorted_areas = sorted(areas, key=lambda a: (a.get("r1", 0), a.get("c1", 0)))
