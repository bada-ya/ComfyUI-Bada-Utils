...
export function isDualManagerEnabled() {
    try {
        if (app?.ui?.settings) {
...
// Find the target anchor container (Extensions button or its outer pill)
export function findAnchorContainer() {
...
function createManagerPillElement() {
...
export function injectButton() {
    ...
    // Clean up any extraneous duplicate pills
    const allExisting = document.querySelectorAll("#bada-dual-manager-pill");
    ...
    // Check if already in DOM and correctly positioned
    if (pill && document.body.contains(pill) && pill.nextElementSibling === anchorEl) {
        pill.style.display = isDualManagerEnabled() ? "inline-flex" : "none";
        return true;
    }

    // Reuse cached pill or create fresh
    if (!pill) {
        pill = cachedPill && !document.body.contains(cachedPill) ? cachedPill : createManagerPillElement();
        cachedPill = pill;
    }
...
export function updateDualManagerVisibility(forceState) {
    ...
    // When enabled, ensure it exists in DOM and is positioned before anchor
...
// Persistent observer and auto-recovery watcher
let observerInitialized = false;

function setupAutoRecoveryObserver() {
    if (observerInitialized) return;
    observerInitialized = true;

    let debounceTimer = null;
    const checkAndReinject = () => {
        if (!isDualManagerEnabled()) return;
        const pill = document.getElementById("bada-dual-manager-pill");
        const anchorEl = findAnchorContainer();
        if (!pill || !document.body.contains(pill) || (anchorEl && pill.nextElementSibling !== anchorEl)) {
            injectButton();
        }
    };
...
export function setupDualManager() {
    ...
    // Fast initial polling for startup
    let attempts = 0;
    ...
    // Setup persistent auto-recovery observer
    setupAutoRecoveryObserver();
}
